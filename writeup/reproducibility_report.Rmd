...
```{r}
#model1 <- lm(num_words ~ lh_latOTS + rh_latOTS + age, data = borst2016 )
```
...
```{r}
#model2 <- lm(num_words ~ age, data = borst2016 )
```
...
```{r}
#model3 <- lm(num_words ~ lh_latOTS + rh_latOTS, data = borst2016 )
```
...
```{r}
#sulcal_pattern <- t.test(num_words ~ group)
```
